...
"""
s2p_gnn_props_max_elf.py
Structure → Peak of ELF with scikit-learn (MAX 参数风格)
...
TARGET_DEFAULT = ["Peak of ELF"]  # 目标列默认值
...
def cmd_train(args):
    ...
    # NaN 报告
    feat_df.isna().mean().sort_values(ascending=False).to_csv(os.path.join(args.save_dir, "nan_ratio_by_feature.csv"))
...
def build_parser():
    p = argparse.ArgumentParser(description='Structure→Peak of ELF with scikit-learn — MAX params style')
    sub = p.add_subparsers(dest='cmd', required=True)

    # helper: optional string that can be passed without value
    ...
    sp.add_argument('--save-dir', type=str, default='runs/sk_max_elf')
    ...
    # predict
    spd = sub.add_parser('predict', help='predict Peak of ELF for all CIFs in a directory')
    spd.add_argument('--model-dir', type=str, required=True)
    spd.add_argument('--in-dir', type=str, required=True)
    spd.add_argument('--out-csv', type=str, default='preds_peak_elf.csv')
