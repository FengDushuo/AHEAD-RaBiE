...
class S2PModel(nn.Module):
    ...
    def __init__(self, emb_dim=128, hidden=256, layers=6, rbf_dim=32, cutoff=6.0, max_z=100, dropout=0.1, out_dim=3, use_virtual_node=True):
        ...
        self.edge_lin = nn.Linear(rbf_dim + 5, hidden)
    ...
    def encode_edges(self, edge_attr):
        ...
        e = torch.cat([rbf, u, dist.unsqueeze(-1), inv], dim=-1)
...
def make_loss(name: str, huber_beta: float):
    name = name.lower()
    if name == 'mse':
        def _loss(pred, tgt): return F.mse_loss(pred, tgt, reduction='none').mean(dim=1)
    elif name == 'mae':
        def _loss(pred, tgt): return F.l1_loss(pred, tgt, reduction='none').mean(dim=1)
    else:  # huber
        def _loss(pred, tgt): return F.smooth_l1_loss(pred, tgt, beta=huber_beta, reduction='none').mean(dim=1)
    return _loss

def train_one_epoch(model, loader, opt, scheduler, y_mean, y_std, device, zn_weight: float, base_loss):
    model.train(); total = 0.0
    for batch in loader:
        batch = batch.to(device)
        y_hat = model(batch.x.long(), batch.pos, batch.edge_index, batch.edge_attr, batch.batch)
        y = (batch.y - y_mean) / y_std
        per = base_loss(y_hat, y)  # (B,)
        if zn_weight > 1.0:
...
def run_once(args, seed_offset=0, tag=""):
    ...
    # 目标变换：Band Gap -> log1p（仅训练用；评估/导出反变换）
    def _forward_y(y_np):
        y = y_np.copy()
    ...
    if args.log1p_bandgap:
        for it in items:
            it.y = _forward_y(it.y.reshape(1, -1))[0]
    ...
    for epoch in range(1, args.epochs+1):
        tr_loss = train_one_epoch(model, train_loader, opt, scheduler, y_mean, y_std, device, args.zn_weight, base_loss)
        va_loss, YT_va, YP_va, _ = eval_epoch(model, val_loader, y_mean, y_std, device, inverse_y_fn=_inverse_y)
    ...
    if args.calibrate and len(val_loader) > 0:
        _, YT_val, YP_val, _ = eval_epoch(model, val_loader, y_mean, y_std, device, inverse_y_fn=_inverse_y)
    ...
    # ----- test -----
    te_loss, YT_te, YP_te, SIDS_te = eval_epoch(model, test_loader, y_mean, y_std, device, inverse_y_fn=_inverse_y)
    if A is not None:
    ...
    return {
        ...
        "targets": targets, "inverse": _inverse_y,
