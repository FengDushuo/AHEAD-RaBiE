...
def aggregate_predictions(df: pd.DataFrame, pred_col: str, id_col: str = "id", method: str = "median", weight_col: Optional[str] = None) -> pd.DataFrame:
    ...
    for gid, g in df.groupby(id_col, dropna=False):
        vals = pd.to_numeric(g[pred_col], errors="coerce").to_numpy(dtype=float)
        ...
        if len(vals) == 0:
            ...
            m = np.isfinite(vals) & np.isfinite(w[:len(vals)])
            pred = float(np.average(vals[m], weights=w[:len(vals)][m])) if m.sum() else float(np.median(vals))
        else:
        ...
        if "target" in g.columns:
            row["target"] = pd.to_numeric(g["target"], errors="coerce").dropna().iloc[0] if pd.to_numeric(g["target"], errors="coerce").notna().any() else np.nan
        rows.append(row)
...
def main() -> None:
    ...
    ap.add_argument("--aggregate-method", default="median", choices=["median", "mean"])
