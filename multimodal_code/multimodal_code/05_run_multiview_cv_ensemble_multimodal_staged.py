...
def parse_args():
    ...
    ap.add_argument("--standardize-target", action="store_true", help="Enable fold-wise target standardization in regression")
    ap.add_argument("--regress-script", default="regress_run_multimodal_staged.py")
    ap.add_argument("--predict-script", default="regress_predict_multimodal.py")
    ap.add_argument("--use-val-calibration", action="store_true", help="Fit validation calibration and apply it to test/addH-out predictions")
    ap.add_argument("--calibration-mode", default="bias_only", choices=["affine", "bias_only"], help="Calibration mapping fitted on validation predictions")
    ap.add_argument("--min-val-weight", type=float, default=1e-6, help="Floor used when converting val MAE to ensemble weights")

    # staged regression training controls
    ap.add_argument("--train-strategy", default="two_stage", choices=["two_stage", "single_stage"])
    ap.add_argument("--stage1-epochs", type=int, default=12)
    ap.add_argument("--stage2-epochs", type=int, default=28)
    ...
    ap.add_argument("--unfreeze-top-n-layers-stage2", type=int, default=4)

    ap.add_argument("--lr-stage1-new", type=float, default=5e-5)
    ap.add_argument("--lr-stage1-text-projection", type=float, default=5e-6)
    ap.add_argument("--lr-stage2-new", type=float, default=2e-5)
    ap.add_argument("--lr-stage2-text-projection", type=float, default=3e-6)
    ap.add_argument("--lr-stage2-text-top", type=float, default=1e-6)
...
def apply_calibration_dict(pred_obj: Dict[str, float], calib: Dict[str, float]) -> Dict[str, float]:
    ...
    for k, v in pred_obj.items():
        out[str(k)] = float(calib["a"] * float(v) + calib["b"])
    return out
...
def main():
            ...
            try:
                ...
                if args.run_predict_test:
                    ...
                    if val_raw_metrics is not None:
                        ...
                        metrics["val_rmse_raw"] = float(val_raw_metrics["rmse"])
                        metrics["val_r2_raw"] = float(val_raw_metrics["r2"])
                    if val_cal_metrics is not None:
                        metrics["val_mae_cal"] = float(val_cal_metrics["mae"])
                        metrics["val_rmse_cal"] = float(val_cal_metrics["rmse"])
                        metrics["val_r2_cal"] = float(val_cal_metrics["r2"])
    ...
    if oof_parts:
        ...
        oof_metrics = metrics_from_df(oof_ens, pred_col="pred")
