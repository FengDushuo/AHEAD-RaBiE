...
A milder target-domain weighting scheme.

Design goals
------------
Compared with the stronger weighting version, this script is intentionally more conservative:
- narrower final weight range
- fewer active components by default
- weaker material / dopant / formula emphasis
- site / anchor / embedding weighting disabled by default

Recommended first use
---------------------
python make_target_domain_weighted_train_table_mild.py \
  --train-csv addH_master_merged_robust.csv \
  --target-csv addH_out_master_normalized.csv \
  --output-csv addH_master_target_weighted_mild.csv \
  --debug-csv target_domain_weight_debug_mild.csv \
  --profile-json target_domain_profile_mild.json

Key defaults
------------
- final clipped weight range: [0.85, 1.35]
- active by default:
    w_material, w_miller, w_dopant, w_formula
- inactive by default:
    w_site, w_anchor, w_emb

Why this version exists
-----------------------
If the previous aggressive target-domain weighting hurt OOF performance, this milder
version is a safer next step. It mostly gives a gentle preference to:
- same material family / same Miller
- similar dopant properties
- oxide-like source samples

while avoiding strong distortion of the original training distribution.
"""
...
def parse_args():
    ...
    ap.add_argument("--train-eq-emb-pkl", default=None)
    ap.add_argument("--target-eq-emb-pkl", default=None)

    ap.add_argument("--output-csv", default="addH_master_target_weighted_mild.csv")
    ap.add_argument("--debug-csv", default="target_domain_weight_debug_mild.csv")
    ap.add_argument("--profile-json", default="target_domain_profile_mild.json")

    # narrower range than the strong version
    ap.add_argument("--min-weight", type=float, default=0.85)
    ap.add_argument("--max-weight", type=float, default=1.35)

    # conservative defaults
    ap.add_argument("--use-material", action="store_true", default=True)
    ap.add_argument("--use-miller", action="store_true", default=True)
    ap.add_argument("--use-site", action="store_true", default=False)
    ap.add_argument("--use-anchor", action="store_true", default=False)
    ...
    # mild material weighting
    ap.add_argument("--material-exact", type=float, default=1.20)
    ap.add_argument("--material-family", type=float, default=1.10)
    ap.add_argument("--material-oxide", type=float, default=1.04)
    ap.add_argument("--material-other", type=float, default=0.98)

    # mild miller weighting
    ap.add_argument("--miller-exact", type=float, default=1.08)
    ap.add_argument("--miller-known-mismatch", type=float, default=1.00)
    ap.add_argument("--miller-unknown", type=float, default=0.97)

    # disabled by default, but kept for completeness
    ap.add_argument("--site-min", type=float, default=0.95)
    ap.add_argument("--site-max", type=float, default=1.08)

    ap.add_argument("--anchor-min", type=float, default=0.96)
    ap.add_argument("--anchor-max", type=float, default=1.06)

    # mild dopant weighting
    ap.add_argument("--dopant-alpha", type=float, default=0.18)
    ap.add_argument("--dopant-min", type=float, default=0.92)
    ap.add_argument("--dopant-max", type=float, default=1.12)

    # mild formula weighting
    ap.add_argument("--formula-oxide", type=float, default=1.04)
    ap.add_argument("--formula-other", type=float, default=0.99)

    # optional emb weighting if explicitly enabled
    ap.add_argument("--emb-min", type=float, default=0.96)
    ap.add_argument("--emb-max", type=float, default=1.08)

    # blend raw weight back toward 1.0 for extra conservativeness
    ap.add_argument("--shrink-to-one", type=float, default=0.55,
                    help="0 means no shrink; 1 means fully collapse to 1.0. "
                         "Applied as w = 1 + (w-1)*(1-shrink_to_one).")
...
def score_dopant(row: pd.Series, target_profile: Dict[str, Any], args) -> float:
    ...
    if block in bf and len(bf) > 0 and bf[block] > 0:
        score *= 1.0 + 0.03 * min(1.0, float(bf[block]) / max(bf.values()))
    return float(score)
...
def main():
    ...
    for _, row in train_df.iterrows():
        ...
        w_domain_raw = wm * wmi * ws * wa * wd * wf * we
        rows.append((wm, wmi, ws, wa, wd, wf, we, w_domain_raw))
    ...
    shrink = float(args.shrink_to_one)
    if shrink > 0:
        out_df["w_domain"] = 1.0 + (out_df["w_domain"] - 1.0) * (1.0 - shrink)
    ...
    out_df[debug_cols].sort_values("w_domain", ascending=False).to_csv(args.debug_csv, index=False)

    profile_out = dict(target_profile)
    profile_out["settings"] = {
        "min_weight": float(args.min_weight),
        "max_weight": float(args.max_weight),
        "shrink_to_one": float(args.shrink_to_one),
        "normalize_to_mean1": bool(args.normalize_to_mean1),
        "use_material": bool(args.use_material),
        "use_miller": bool(args.use_miller),
        "use_site": bool(args.use_site),
        "use_anchor": bool(args.use_anchor),
        "use_dopant": bool(args.use_dopant),
        "use_formula": bool(args.use_formula),
