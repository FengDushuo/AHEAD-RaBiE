...
def parse_args():
    ...
    ap.add_argument("--clean-run-dirs", action="store_true", help="Remove existing per-run clip/regress checkpoint folders before training. Recommended for experiment grids and reruns.")
    ...
    ap.add_argument("--projection-dim", type=int, default=256, help="Regression projection dim. Clip stage still uses raw eq_dim unless clip-projection-dim is set.")
    ap.add_argument("--clip-projection-dim", type=int, default=0, help="Optional clip-stage projection dim. 0 means use raw eq_dim to stay compatible with original clip_run.")
    ap.add_argument("--dropout-rate", type=float, default=0.10)

    # AddH model-head variants for regress_run_multimodal_staged_aligned_v2.py
    ap.add_argument("--regress-model-variant", default="gated_sum",
                    choices=["gated_sum", "concat_interact", "residual_graph", "graph_only", "text_only"],
                    help="Fusion head variant used by the v2 regression/prediction scripts.")
    ap.add_argument("--fusion-hidden-mult", type=float, default=2.0)
    ap.add_argument("--regressor-hidden-mult", type=float, default=1.0)
    ap.add_argument("--regressor-depth", type=int, default=2)
    ap.add_argument("--graph-proj-depth", type=int, default=2)
    ap.add_argument("--graph-hidden-mult", type=float, default=1.0)
    ap.add_argument("--graph-noise-std", type=float, default=0.0)
    ...
    ap.add_argument(
        "--max-val-mae-for-ensemble",
        type=float,
        default=None,
        help="Optional reliability filter: exclude runs with calibrated validation MAE above this threshold from OOF/addH-out ensembling. Individual run outputs are still saved.",
    )
    ap.add_argument(
        "--sanitize-target-abs-max",
        type=float,
        default=None,
        help="Hard safety filter applied to existing CV pkl files before training/prediction. Rows in clip_train/clip_val/regress_train/regress_val/regress_test with abs(target) above this value are removed in-place. This protects against stale CV splits that still contain parse/energy outliers.",
    )
    ap.add_argument(
        "--sanitize-target-outliers",
        action="store_true",
        help="Also remove rows with outlier_flag_target=True from existing CV pkl files before training/prediction.",
    )
...
def _sanitize_one_pickle(path: Path, target_abs_max: Optional[float], drop_outlier_flags: bool) -> dict:
    if not path.exists():
        return {"path": str(path), "exists": False, "before": 0, "after": 0, "dropped_abs": 0, "dropped_flag": 0}
    df = pd.read_pickle(path)
    before = int(len(df))
    if before == 0 or "target" not in df.columns:
        return {"path": str(path), "exists": True, "before": before, "after": before, "dropped_abs": 0, "dropped_flag": 0}
    ...
    if target_abs_max is not None:
        ...
        # Target-domain CLIP rows use target=0.0 and are retained; nonnumeric targets are removed.
    ...
    if drop_outlier_flags and "outlier_flag_target" in df.columns:
        flag_keep = ~_coerce_bool_series(df["outlier_flag_target"])
    ...
    if after == 0:
        raise ValueError(
            f"Sanitizing {path} would remove all rows (before={before}, target_abs_max={target_abs_max}, "
            f"drop_outlier_flags={drop_outlier_flags}). Please inspect the target column and threshold."
        )
    if after != before:
        ...
        if not backup.exists():
            df.to_pickle(backup)
        out.to_pickle(path)
    return {"path": str(path), "exists": True, "before": before, "after": after, "dropped_abs": dropped_abs, "dropped_flag": dropped_flag}


def sanitize_cv_pickles(cv_root: Path, fold_dirs: List[Path], target_abs_max: Optional[float], drop_outlier_flags: bool) -> None:
    if target_abs_max is None and not drop_outlier_flags:
        return
    print("[INFO] Sanitizing existing CV pickle files before training/prediction...")
    print("[INFO] sanitize_target_abs_max =", target_abs_max)
    print("[INFO] sanitize_target_outliers =", bool(drop_outlier_flags))
    rows = []
    for fold_dir in fold_dirs:
        for name in ["clip_train.pkl", "clip_val.pkl", "regress_train.pkl", "regress_val.pkl", "regress_test.pkl"]:
            rows.append(_sanitize_one_pickle(fold_dir / name, target_abs_max, drop_outlier_flags))
    report = pd.DataFrame(rows)
    report_path = cv_root / "cv_pickle_sanitize_report.csv"
    report.to_csv(report_path, index=False)
    total_dropped_abs = int(report.get("dropped_abs", pd.Series(dtype=int)).sum())
    total_dropped_flag = int(report.get("dropped_flag", pd.Series(dtype=int)).sum())
    print("[INFO] CV pickle sanitize report ->", report_path)
    print("[INFO] dropped by abs(target) threshold =", total_dropped_abs)
    print("[INFO] dropped by outlier flag =", total_dropped_flag)
...
def switch_model_cfg_only(repo_root: Path, model_cfg: dict):
    write_yaml(repo_root / "model" / "clip.yml", model_cfg)
...
def main():
    ...
    sanitize_cv_pickles(
        cv_root=cv_root,
        fold_dirs=fold_dirs,
        target_abs_max=args.sanitize_target_abs_max,
        drop_outlier_flags=bool(args.sanitize_target_outliers),
    )
    ...
    for fold_dir in fold_dirs:
        ...
        clip_proj_dim = int(args.clip_projection_dim) if int(args.clip_projection_dim) > 0 else int(eq_dim)
        clip_model_cfg = build_model_cfg(eq_dim, args.roberta_pretrain, clip_proj_dim, args.dropout_rate)
        regress_model_cfg = build_model_cfg(eq_dim, args.roberta_pretrain, int(args.projection_dim), args.dropout_rate)
        ...
        for seed in seeds:
            ...
            if args.clean_run_dirs:
                if args.run_clip:
                    shutil.rmtree(clip_ckpt_base, ignore_errors=True)
                if args.run_regress:
                    shutil.rmtree(regress_ckpt_base, ignore_errors=True)
            clip_ckpt_base.mkdir(exist_ok=True)
            regress_ckpt_base.mkdir(exist_ok=True)

            # clip_run.py/regress_run.py append only minute-level timestamps to run_name.
            # In grid/rerun settings, repeated launches in the same minute can collide.
            # This suffix makes checkpoint directories unique and prevents FileExistsError.
            unique_suffix = f"pid{os.getpid()}_ns{time.time_ns()}"

            clip_cfg = {
                "run_name": f"addH_clip_{run_tag}_{unique_suffix}",
            ...
            regress_cfg = {
                "run_name": f"addH_regress_mm_{run_tag}_{unique_suffix}",
                ...
                "regress_model_variant": str(args.regress_model_variant),
                "fusion_hidden_mult": float(args.fusion_hidden_mult),
                "regressor_hidden_mult": float(args.regressor_hidden_mult),
                "regressor_depth": int(args.regressor_depth),
                "graph_proj_depth": int(args.graph_proj_depth),
                "graph_hidden_mult": float(args.graph_hidden_mult),
                "graph_noise_std": float(args.graph_noise_std),
            }

            patch_root_yml(repo_root, clip_cfg, regress_cfg, clip_model_cfg)
            try:
                if args.run_clip:
                ...
                # clip_run.py in the original repo expects text projection dim to match raw eq_emb dim.
                # So clip stage uses clip_model_cfg (usually projection_dim == eq_dim).
                # Regression stage can switch to a smaller projection dim because the aligned
                # regression script adds an explicit GraphProjectionHead.
                switch_model_cfg_only(repo_root, regress_model_cfg)
                ...
                if args.run_predict_test:
                    ...
                    test_df["val_mae_cal"] = float(val_cal_metrics["mae"]) if val_cal_metrics is not None else np.nan
                    test_df["val_mae_raw"] = float(val_raw_metrics["mae"]) if val_raw_metrics is not None else np.nan
                ...
                if args.run_predict_out:
                    ...
                    merged["val_mae_cal"] = float(val_cal_metrics["mae"]) if val_cal_metrics is not None else np.nan
                    merged["val_mae_raw"] = float(val_raw_metrics["mae"]) if val_raw_metrics is not None else np.nan
    ...
    if all_run_metrics:
        ...
        overall = {
            ...
            "clip_projection_dim": int(args.clip_projection_dim) if int(args.clip_projection_dim) > 0 else int(eq_dim),
        }
        save_json(work_dir / "metrics_summary_overall.json", overall)

    def _filter_ensemble_runs(df: pd.DataFrame, label: str) -> pd.DataFrame:
        if args.max_val_mae_for_ensemble is None or "val_mae_cal" not in df.columns:
            return df
        thr = float(args.max_val_mae_for_ensemble)
        run_info = df[["run_tag", "val_mae_cal"]].drop_duplicates().copy()
        keep_runs = set(run_info.loc[run_info["val_mae_cal"].astype(float) <= thr, "run_tag"].astype(str))
        if not keep_runs:
            print(f"[WARN] {label}: all runs exceed --max-val-mae-for-ensemble={thr}; keeping all runs instead.")
            return df
        dropped = sorted(set(run_info["run_tag"].astype(str)) - keep_runs)
        if dropped:
            print(f"[INFO] {label}: dropped {len(dropped)} weak ensemble runs by val_mae_cal>{thr}: {dropped}")
        return df[df["run_tag"].astype(str).isin(keep_runs)].copy()

    if oof_parts:
        oof_all = pd.concat(oof_parts, axis=0, ignore_index=True)
        oof_all.to_csv(work_dir / "test_pred_all_runs.csv", index=False)
        oof_for_ens = _filter_ensemble_runs(oof_all, "OOF")
        oof_for_ens.to_csv(work_dir / "test_pred_runs_used_for_ensemble.csv", index=False)
        oof_ens_parts = []
        for fold_idx, sub in oof_for_ens.groupby("fold"):
    ...
    if out_parts:
        ...
        out_for_ens = _filter_ensemble_runs(out_all, "addH-out")
        out_for_ens.to_csv(work_dir / "addH_out_pred_runs_used_for_ensemble.csv", index=False)
        out_ens = aggregate_preds(out_for_ens[["id", "pred", "run_weight"]].copy(), pred_col="pred", method=args.ensemble_method, weight_col="run_weight")
        manifest = out_for_ens.drop(columns=["pred"], errors="ignore").drop_duplicates("id")
