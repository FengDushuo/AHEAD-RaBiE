...
class MultimodalRegressionModel(nn.Module):
    def __init__(self, config: dict, graph_input_dim: int):
        ...
        self.graph_projection = GraphProjectionHead(input_dim=graph_input_dim, projection_dim=projection_dim, dropout_rate=dropout_rate)
...
def main():
    ...
    with torch.no_grad():
        for batch in tqdm(loader):
            ...
            out = model.regressor(fused).squeeze(-1)
