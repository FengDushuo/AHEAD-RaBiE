...
Train CatBoost on single-structure data built by:
  04_make_singleview_data_cv_meta.py

Features used
-------------
- Graph embedding PCA features from eq_emb
...
Global:
  ...
  metrics_summary_overall.json
  test_pred_all_runs.csv
  test_pred_oof_ensemble.csv
  test_pred_oof_ensemble_metrics.json
...
def _build_catboost_frame(df: pd.DataFrame, graph_feat: np.ndarray, graph_cols: List[str], cat_cols: List[str], num_cols: List[str], text_cols: List[str]) -> pd.DataFrame:
...
def main():
    ...
    for fold_dir in fold_dirs:
        ...
        for seed in seeds:
            ...
            feature_cols = graph_cols + [c for c in num_cols if c in tr.columns] + [c for c in cat_cols if c in tr.columns] + [c for c in text_cols if c in tr.columns]
            cat_feature_names = [c for c in cat_cols if c in tr.columns]
            text_feature_names = [c for c in text_cols if c in tr.columns]
