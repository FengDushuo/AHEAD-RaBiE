...
# -*- coding: utf-8 -*-
"""
04_make_singleview_data_cv_meta_v2_mv.py

Updated singleview CV builder with support for merging multiview meta features.
"""
...
def parse_args():
    ...
    ap.add_argument("--multiview-train-features-csv", default=None, help="Optional per-id multiview features for source training rows")
    ap.add_argument("--multiview-out-features-csv", default=None, help="Optional per-id multiview features for target/out rows")
    ap.add_argument("--multiview-prefix", default="mv_", help="Prefix used to identify multiview columns in summary/debug output")
...
def _load_multiview_feature_csv(path: str, kind: str) -> pd.DataFrame:
    p = Path(path)
    if not p.exists():
        raise FileNotFoundError(p)
    df = pd.read_csv(p)
    if "id" not in df.columns:
        raise ValueError(f"{p}: missing id column for {kind} multiview features")
    df["id"] = df["id"].astype(str)
    df = df.drop_duplicates(subset=["id"], keep="last")
    return df


def _merge_multiview_features(df: pd.DataFrame, mv_df: pd.DataFrame | None) -> Tuple[pd.DataFrame, List[str]]:
    if mv_df is None:
        return df.copy(), []
    out = df.copy()
    before_cols = set(out.columns)
    out["id"] = out["id"].astype(str)
    out = out.merge(mv_df, on="id", how="left")
    new_cols = [c for c in out.columns if c not in before_cols and c != "id"]
    return out, new_cols


def _prepare_master(df: pd.DataFrame, emb: Dict[str, object], text_col: str, strict: bool, mv_train_df: pd.DataFrame | None):
    ...
    df, mv_cols = _merge_multiview_features(df, mv_train_df)

    df, dop_num_cols, dop_cat_cols = _derive_dopant_features(df)
    df, form_num_cols, form_cat_cols = _derive_formula_features(df)

    reserved_cols = {"id", "target", "text_raw", "text_structured", "eq_emb", "site_type_label", "family_label", "target_bin"}
    ...
    auto_extra_num, auto_extra_cat = [], []
    for c in mv_cols:
        if c in reserved_cols:
            continue
        s = df[c]
        if pd.api.types.is_numeric_dtype(s):
            auto_extra_num.append(c)
        else:
            auto_extra_cat.append(c)

    cat_cols += auto_extra_cat
    num_cols += auto_extra_num
    ...
    meta = {
        ...
        "multiview_cols": mv_cols,
        "multiview_num_cols": auto_extra_num,
        "multiview_cat_cols": auto_extra_cat,
    }
    return usable, meta


def _prepare_addhout(df: pd.DataFrame, emb: Dict[str, object], text_col: str, mv_out_df: pd.DataFrame | None):
    ...
    df["eq_emb"] = df["id"].astype(str).map(emb)
    df, _mv_cols = _merge_multiview_features(df, mv_out_df)
...
def _save_fold(
    ...
    base_cols = ["id", "target", "text_raw", "text_structured", "eq_emb", "site_type_label", "family_label", "target_bin"]
    base_cols += [c for c in meta_cols["cat_cols"] if c in split_df.columns]
    base_cols += [c for c in meta_cols["num_cols"] if c in split_df.columns]
    ...
    if addhout_df is not None and len(addhout_df) > 0:
        out_cols = ["id", "target", "text_raw", "text_structured", "eq_emb"]
        out_cols += [c for c in meta_cols["cat_cols"] if c in addhout_df.columns]
        out_cols += [c for c in meta_cols["num_cols"] if c in addhout_df.columns]
    ...
    meta = {
        ...
        "multiview_cols": meta_cols.get("multiview_cols", []),
        "multiview_num_cols": meta_cols.get("multiview_num_cols", []),
        "multiview_cat_cols": meta_cols.get("multiview_cat_cols", []),
    }
...
def main():
    ...
    mv_train_df = _load_multiview_feature_csv(args.multiview_train_features_csv, "train") if args.multiview_train_features_csv else None
    mv_out_df = _load_multiview_feature_csv(args.multiview_out_features_csv, "out") if args.multiview_out_features_csv else None

    addh = pd.read_csv(args.addh_master_csv)
    emb = _load_emb(Path(args.eq_emb_pkl))
    usable_addh, meta = _prepare_master(addh, emb, text_col=args.text_col, strict=args.strict, mv_train_df=mv_train_df)
    ...
    if args.addhout_master_csv and args.addhout_eq_emb_pkl:
        ...
        addhout_usable = _prepare_addhout(addhout, addhout_emb, text_col=args.text_col, mv_out_df=mv_out_df)
    ...
    info = {
        ...
        "multiview_train_features_csv": args.multiview_train_features_csv,
        "multiview_out_features_csv": args.multiview_out_features_csv,
        "multiview_prefix": args.multiview_prefix,
        "multiview_cols": meta.get("multiview_cols", []),
        "multiview_num_cols": meta.get("multiview_num_cols", []),
        "multiview_cat_cols": meta.get("multiview_cat_cols", []),
    }
    ...
    print("[INFO] multiview_cols =", meta.get("multiview_cols", []))
