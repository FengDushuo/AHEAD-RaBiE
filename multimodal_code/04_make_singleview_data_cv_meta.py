...
Build single-structure CV datasets for:
1) NN-style singleview strong model
2) CatBoost tabular/text model

Input
-----
- addH master CSV (merged robust table)
- eq embedding pickle: dict[id] -> addH graph embedding
- optional addH-out master CSV
- optional addH-out eq embedding pickle
...
Notes
-----
- This is still a "single-structure" route: only addH structure embedding is used.
- The script adds stronger metadata features for downstream fusion/CatBoost.
"""
...
def _derive_dopant_features(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    cols_num = []
    cols_cat = []
...
def _derive_formula_features(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    counts_total = []
    n_species = []
    host_heavy = []
...
def _prepare_master(df: pd.DataFrame, emb: Dict[str, object], text_col: str, strict: bool):
    df = df.copy()
    required = ["id", "target", text_col]
    miss = [c for c in required if c not in df.columns]
    if miss:
        raise ValueError(f"Missing required columns in master CSV: {miss}")

    df["eq_emb"] = df["id"].astype(str).map(emb)
    df["text_raw"] = df[text_col].astype(str)
    df["text_structured"] = df.apply(lambda r: _text_structured(r, text_col=text_col), axis=1)
    df["text_len"] = df["text_raw"].fillna("").astype(str).map(lambda x: len(x))
    df["has_known_miller"] = df.get("miller", pd.Series([np.nan] * len(df))).fillna("").astype(str).map(lambda x: 0 if "?" in x or not x.strip() else 1)
    ...
    # stable metadata
    cat_cols = []
    for c in ["family_base", "family_base_miller", "dopant", "site_type", "miller", "data_source", "slab_formula"]:
        if c in df.columns:
    ...
    if "family_base_miller" in df.columns:
...
def _prepare_addhout(df: pd.DataFrame, emb: Dict[str, object], text_col: str):
    df = df.copy()
    required = ["id", text_col]
    miss = [c for c in required if c not in df.columns]
    if miss:
        raise ValueError(f"Missing required columns in addH-out CSV: {miss}")
...
def _save_fold(
    ...
    # derive target_bin on train quantiles only
    ...
    if addhout_df is not None and len(addhout_df) > 0:
        out_cols = ["id", "text_raw", "text_structured", "eq_emb"] + [c for c in meta_cols["cat_cols"] if c in addhout_df.columns] + [c for c in meta_cols["num_cols"] if c in addhout_df.columns]
