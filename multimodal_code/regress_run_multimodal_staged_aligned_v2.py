...
def _new_head_modules(model: MultimodalRegressionModel):
    modules = [model.graph_projection, model.regressor]
    for name in ["fusion_gate", "concat_fusion", "residual_fusion"]:
...
def apply_stage_freeze_policy(model: MultimodalRegressionModel, config: dict, stage_name: str):
    if stage_name == "stage1":
        ...
        for m in _new_head_modules(model):
            set_module_requires_grad(m, True)
...
def build_stage_optimizer(config: dict, model: MultimodalRegressionModel, stage_name: str):
    ...
    if stage_name == "stage1":
        ...
        new_params = []
        for m in _new_head_modules(model):
            new_params += list(_named_trainable_params(m))
        if hasattr(model, "residual_scale") and model.residual_scale.requires_grad:
            new_params.append(model.residual_scale)
        if new_params:
...
def run_regression(config_file: str = "regress_train.yml"):
    ...
    print(f"Regress model variant: {config.get('regress_model_variant', config.get('model_variant', 'gated_sum'))}")
    print(f"Fusion hidden mult: {config.get('fusion_hidden_mult', 2.0)}")
    print(f"Regressor depth: {config.get('regressor_depth', 2)}")
    print(f"Graph noise std: {config.get('graph_noise_std', 0.0)}")
    ...
    model_config["AddHRegressConfig"] = {
        "model_variant": str(config.get("regress_model_variant", config.get("model_variant", "gated_sum"))),
        "fusion_hidden_mult": float(config.get("fusion_hidden_mult", 2.0)),
        "regressor_hidden_mult": float(config.get("regressor_hidden_mult", 1.0)),
        "regressor_depth": int(config.get("regressor_depth", 2)),
        "graph_proj_depth": int(config.get("graph_proj_depth", 2)),
        "graph_hidden_mult": float(config.get("graph_hidden_mult", 1.0)),
        "graph_noise_std": float(config.get("graph_noise_std", 0.0)),
    }
