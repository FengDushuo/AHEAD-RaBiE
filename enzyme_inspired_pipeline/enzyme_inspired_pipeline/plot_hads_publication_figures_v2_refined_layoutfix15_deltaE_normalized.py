...
TARGET_NAME_MAP = {
    "H_adsorption_free_energy_value_eV": r"$\Delta G_{H^*}$ (eV)",
    "H_adsorption_energy_value_eV": r"$\Delta E_{H^*}$ (eV)",
    ...
    "H_adsorption_free_energy_value": r"$\Delta G_{H^*}$",
    "H_adsorption_energy_value": r"$\Delta E_{H^*}$",
...
def target_tick_label(name: str) -> str:
    tick_map = {
        "H_adsorption_free_energy_value_eV": r"$\Delta G_{H^*}$" + "\n(eV)",
        "H_adsorption_energy_value_eV": r"$\Delta E_{H^*}$" + "\n(eV)",
...
def plot_overview_dashboard(
    ...
    mech_cols = [
        "d_band_center", "Bader_charge", "coordination_number", "oxidation_state_value", "oxidation_state",
...
def plot_mechanism_coverage(
    ...
    colors = plt.cm.YlGnBu(norm(df["coverage"].values))
...
def plot_corr_bubble_matrix_v2(
    ...
    if xs:
        sc = ax.scatter(xs, ys, s=sizes, c=colors, cmap="coolwarm", vmin=-1, vmax=1,
                        edgecolor=PALETTE["dark"], linewidth=0.28, alpha=0.92, zorder=3)
    else:
        sc = ScalarMappable(norm=Normalize(-1, 1), cmap="coolwarm")
...
def plot_deltae_ml_importance(
    ...
    ax.set_xlabel("Permutation importance for ΔE_H* prediction")
    title = "Model-based feature importance for ΔE_H*"
...
def plot_dft_priority_v2(dft_df: pd.DataFrame, outdir: Path, dpi: int, formats: Sequence[str], topn: int = 20):
    ...
    colors = plt.cm.viridis(Normalize(vmin=vmin, vmax=vmax)(score))
...
def plot_priority_scatter_v2(
    ...
    sc = ax.scatter(use["x_plot"], use["y"], c=cvals, cmap="viridis", s=30,
...
def main():
    ...
    force_features = parse_comma_list(args.force_features)
