...
2) Descriptor extraction: for each extracted target record, link same-system/site
   atom-resolved DFT descriptors split into H-binding oxygen/anchor-site
   descriptors and neighbouring-metal/surface descriptors.
...
RE_BAND_GAP = re.compile(r"(?:band gap|bandgap|E[_\- ]?g)" + SEP + r"([-+−–]?\d+(?:\.\d+)?)\s*(?:eV)?", re.I)
RE_OH_BOND = re.compile(r"(?:O[-–—]?H bond length|O[-–—]?H distance|d\s*\(?O[-–—]?H\)?)" + SEP + r"([-+−–]?\d+(?:\.\d+)?)\s*(?:Å|A|angstrom)?", re.I)
RE_MO_DIST = re.compile(r"(?:M[-–—]?O distance|M[-–—]?O bond length|metal[-–—]?O distance|metal[-–—]?oxygen distance)" + SEP + r"([-+−–]?\d+(?:\.\d+)?)\s*(?:Å|A|angstrom)?", re.I)
...
h_binding_anchor_atom, h_binding_site_label,
...
o_site_coordination_number, o_site_bader_charge, o_site_charge_transfer,
o_h_bond_length, o_site_descriptor_claim,

neighbor_metal_element, neighbor_metal_site_label,
metal_coordination_number, metal_oxidation_state_raw, metal_oxidation_state_element,
metal_oxidation_state, metal_bader_charge, metal_d_band_center, metal_o_distance,
work_function, band_gap,

coordination_number, oxidation_state_raw, oxidation_state_element, oxidation_state_value,
bader_charge, d_band_center,
vacancy_nearby, hydroxylated_surface, charge_transfer_direction, bridge_structure_nearby,
ELF_descriptor, PZC, bridge_structure, M_O_X_configuration,
descriptor_claim, descriptor_atom_scope, descriptor_assignment_confidence,
descriptor_assignment_note, descriptor_link_method, descriptor_link_confidence, evidence_ids[].

Rules:
- Link descriptors ONLY when they belong to the same material/system/site as the target record.
- Split descriptors into two groups:
  Group 1: H-binding site descriptors. These describe the atom that directly binds H*. For oxide surfaces this is often a surface O atom forming an O-H bond. Put O-site coordination number, O-site Bader charge, O-site charge transfer and O-H bond length here ONLY if the evidence explicitly assigns the value to the H-binding oxygen or adsorption anchor atom.
  Group 2: neighbouring metal/surface descriptors. These describe metal atoms, dopant centres or surface-level electronic properties around the H-binding site. Put metal coordination number, metal oxidation state, metal Bader charge, metal d-band center, metal-O distance, work function and band gap here ONLY if the evidence assigns the value to a metal/dopant/slab/surface environment.
- Do NOT place a metal descriptor into O-site fields. Do NOT place an oxygen descriptor into metal fields.
- d-band center and oxidation state should normally be metal/surface descriptors, not O-site descriptors.
- Work function and band gap are surface/global descriptors, not single-atom descriptors.
- If the descriptor assignment is unclear, keep atom-resolved fields null and, if necessary, place the value only in generic fields with descriptor_atom_scope='generic_or_unclear'.
...
- descriptor_assignment_confidence should be one of: explicit, inferred_same_site, generic_or_unclear, none.
""".strip()
...
def has_any_dft_descriptor(rec: Dict[str, Any]) -> int:
    keys = [
        "coordination_number", "oxidation_state_value", "bader_charge", "d_band_center", "work_function", "PZC",
        "o_site_coordination_number", "o_site_bader_charge", "o_site_charge_transfer", "o_h_bond_length",
        "metal_coordination_number", "metal_oxidation_state", "metal_bader_charge", "metal_d_band_center", "metal_o_distance", "band_gap",
        "vacancy_nearby", "hydroxylated_surface", "bridge_structure_nearby", "ELF_descriptor", "charge_transfer_direction"
    ]
...
def finalize_qc_record(rec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    rec["h_binding_anchor_atom"] = normalize_anchor_atom(rec.get("h_binding_anchor_atom") or rec.get("active_atom") or rec.get("site_label") or rec.get("adsorption_configuration"))
    rec["h_binding_site_label"] = sanitize_site_like(rec.get("h_binding_site_label")) or rec.get("site_label") or rec.get("adsorption_configuration")
    ...
    rec["qc_has_h_binding_site_descriptor"] = int(any(rec.get(k) not in (None, "", [], {}) for k in ["o_site_coordination_number", "o_site_bader_charge", "o_site_charge_transfer", "o_h_bond_length"]))
    rec["qc_has_neighbor_metal_descriptor"] = int(any(rec.get(k) not in (None, "", [], {}) for k in ["metal_coordination_number", "metal_oxidation_state", "metal_bader_charge", "metal_d_band_center", "metal_o_distance", "work_function", "band_gap"]))
    rec["qc_descriptor_atom_resolved"] = int(rec["qc_has_h_binding_site_descriptor"] or rec["qc_has_neighbor_metal_descriptor"])
...
def heuristic_targets(paper_id: str, evs: List[Dict[str, Any]], relpath: str, max_abs_ev: float) -> List[Dict[str, Any]]:
    ...
    for i, m in enumerate(RE_H_E.finditer(txt), 1):
        ...
        raw = {
            ...
            "h_binding_anchor_atom": normalize_anchor_atom(RE_SITE.search(win).group(1) if RE_SITE.search(win) else None),
            "h_binding_site_label": RE_SITE.search(win).group(1) if RE_SITE.search(win) else None,
...
def normalize_anchor_atom(x: Any) -> Optional[str]:
    """Normalize the atom that directly anchors H*."""
    s0 = clean_scalar(x)
    if not s0:
        return None
    s = s0.lower().replace("oxygen", "o").replace("metal", "m")
    if re.search(r"\bo\b|surface o|o[- ]top|o[- ]h|oxygen", s):
        return "O"
    if re.search(r"bridge", s):
        return "bridge"
    # common metal symbols/labels
    if re.search(r"\b(fe|co|ni|cu|zn|ce|pt|pd|ru|rh|ir|mo|w|mn|cr|ti|zr|au|ag|m)\b|metal", s):
        return "metal"
    if re.search(r"vacancy", s):
        return "vacancy"
    return s0

def normalize_assignment_confidence(x: Any) -> str:
    s = normalize_text(x).strip().lower().replace("-", "_").replace(" ", "_")
    allowed = {"explicit", "inferred_same_site", "generic_or_unclear", "none"}
    return s if s in allowed else "generic_or_unclear"

def infer_neighbor_metal_element(*vals: Any) -> Optional[str]:
    text = " ".join(normalize_text(v) for v in vals if v is not None)
    for el in sorted(ELEMENT_SYMBOLS, key=len, reverse=True):
        if el in {"H","O","N","C","S","P","B","F","Cl","Br","I"}:
            continue
        if re.search(rf"(?<![A-Za-z]){re.escape(el)}(?![a-z])", text):
            return el
    return None

def atom_resolved_has_desc(rec: Dict[str, Any]) -> int:
    keys = [
        "o_site_coordination_number", "o_site_bader_charge", "o_site_charge_transfer", "o_h_bond_length",
        "metal_coordination_number", "metal_oxidation_state", "metal_bader_charge", "metal_d_band_center",
        "metal_o_distance", "work_function", "band_gap"
    ]
    return int(any(rec.get(k) not in (None, "", [], {}) for k in keys))
...
def descriptor_from_window(win: str) -> Dict[str, Any]:
    ...
    cn = to_num(RE_CN.search(win).group(1)) if RE_CN.search(win) else None
    bader = to_num(RE_BADER.search(win).group(1)) if RE_BADER.search(win) else None
    dband = to_num(RE_DBAND.search(win).group(1)) if RE_DBAND.search(win) else None
    wf = to_num(RE_WORK.search(win).group(1)) if RE_WORK.search(win) else None
    bg = to_num(RE_BAND_GAP.search(win).group(1)) if RE_BAND_GAP.search(win) else None
    oh = to_num(RE_OH_BOND.search(win).group(1)) if RE_OH_BOND.search(win) else None
    mo = to_num(RE_MO_DIST.search(win).group(1)) if RE_MO_DIST.search(win) else None
    return {
        # Generic backward-compatible fields
        ...
        # Atom-resolved fields. Heuristic fallback is conservative: electronic metal descriptors
        # are placed in metal/surface fields, while O-site fields are only filled for O-H bond length.
        "h_binding_anchor_atom": normalize_anchor_atom(RE_SITE.search(win).group(1) if RE_SITE.search(win) else None),
        "h_binding_site_label": RE_SITE.search(win).group(1) if RE_SITE.search(win) else None,
        "o_site_coordination_number": None,
        "o_site_bader_charge": None,
        "o_site_charge_transfer": None,
        "o_h_bond_length": oh,
        "neighbor_metal_element": infer_neighbor_metal_element(win),
        "neighbor_metal_site_label": None,
        "metal_coordination_number": cn,
        "metal_oxidation_state_raw": ox_raw2,
        "metal_oxidation_state_element": ox_el,
        "metal_oxidation_state": ox_val,
        "metal_bader_charge": None,
        "metal_d_band_center": dband,
        "metal_o_distance": mo,
        "band_gap": bg,
        "descriptor_atom_scope": "heuristic_metal_or_surface",
        "descriptor_assignment_confidence": "generic_or_unclear",
        "descriptor_assignment_note": "Heuristic fallback; atom assignment should be treated cautiously.",
    }
...
def normalize_descriptor(raw: Dict[str, Any], evs: List[Dict[str, Any]], ev_text_all: str) -> Dict[str, Any]:
    ...
    # Generic oxidation-state parsing for backward compatibility.
    ...
    # Metal oxidation-state parsing for atom-resolved output.
    m_ox_raw = clean_scalar(raw.get("metal_oxidation_state_raw") or raw.get("metal_oxidation_state") or ox_raw)
    m_ox_el = clean_scalar(raw.get("metal_oxidation_state_element") or raw.get("neighbor_metal_element") or ox_el2 or ox_el)
    m_ox_raw2, m_ox_el2, m_ox_val2 = parse_oxidation_state(m_ox_raw, m_ox_el)
    m_ox_val = to_num(raw.get("metal_oxidation_state"))
    if m_ox_val is None:
        m_ox_val = m_ox_val2 if m_ox_val2 is not None else ox_val
    ...
    h_anchor = normalize_anchor_atom(raw.get("h_binding_anchor_atom") or raw.get("active_atom") or raw.get("site_label"))

    # Atom-resolved values from the LLM. Generic values are kept but are not blindly
    # assigned to O-site fields unless the LLM explicitly did so.
    o_cn = to_num(raw.get("o_site_coordination_number"))
    o_bader = to_num(raw.get("o_site_bader_charge"))
    o_ct = clean_scalar(raw.get("o_site_charge_transfer"))
    oh_len = to_num(raw.get("o_h_bond_length"))

    metal_cn = to_num(raw.get("metal_coordination_number"))
    metal_bader = to_num(raw.get("metal_bader_charge"))
    metal_dband = to_num(raw.get("metal_d_band_center"))
    if metal_dband is None:
        metal_dband = to_num(raw.get("d_band_center"))
    metal_o_dist = to_num(raw.get("metal_o_distance"))
    band_gap = to_num(raw.get("band_gap"))

    # Generic backward-compatible fields.
    generic_cn = to_num(raw.get("coordination_number"))
    generic_bader = to_num(raw.get("bader_charge"))
    generic_dband = to_num(raw.get("d_band_center")) or metal_dband
    if metal_cn is None and generic_cn is not None:
        # Coordination number is more often reported for metal centres in catalyst papers;
        # keep O-site fields null unless explicitly assigned.
        metal_cn = generic_cn
    if metal_bader is None and generic_bader is not None and re.search(r"metal|M|dopant|Fe|Co|Ni|Cu|Zn|Ce|Pt|Pd|Ru|Rh|Ir|Mo|W|Mn|Cr|Ti|Zr", ev_txt, re.I):
        metal_bader = generic_bader

    d = {
        "target_id": clean_scalar(raw.get("target_id")),
        "site_label": sanitize_site_like(raw.get("site_label")),
        "active_atom": sanitize_site_like(raw.get("active_atom")) or h_anchor or ox_el2 or ox_el,
        "h_binding_anchor_atom": h_anchor,
        "h_binding_site_label": sanitize_site_like(raw.get("h_binding_site_label")) or sanitize_site_like(raw.get("site_label")),

        "o_site_coordination_number": o_cn,
        "o_site_bader_charge": o_bader,
        "o_site_charge_transfer": o_ct,
        "o_h_bond_length": oh_len,
        "o_site_descriptor_claim": clean_scalar(raw.get("o_site_descriptor_claim")),

        "neighbor_metal_element": clean_scalar(raw.get("neighbor_metal_element")) or infer_neighbor_metal_element(raw.get("neighbor_metal_site_label"), m_ox_el2, m_ox_el, ev_txt),
        "neighbor_metal_site_label": sanitize_site_like(raw.get("neighbor_metal_site_label")),
        "metal_coordination_number": metal_cn,
        "metal_oxidation_state_raw": m_ox_raw2 or m_ox_raw,
        "metal_oxidation_state_element": m_ox_el2 or m_ox_el,
        "metal_oxidation_state": m_ox_val,
        "metal_bader_charge": metal_bader,
        "metal_d_band_center": metal_dband,
        "metal_o_distance": metal_o_dist,
        "work_function": to_num(raw.get("work_function")),
        "band_gap": band_gap,

        # Backward-compatible generic columns.
        "coordination_number": metal_cn if metal_cn is not None else (o_cn if o_cn is not None else generic_cn),
        "vacancy_nearby": bool_presence(raw.get("vacancy_nearby")),
        "hydroxylated_surface": bool_presence(raw.get("hydroxylated_surface")),
        "oxidation_state_raw": m_ox_raw2 or ox_raw2 or ox_raw,
        "oxidation_state_element": m_ox_el2 or ox_el2 or ox_el,
        "oxidation_state_value": m_ox_val if m_ox_val is not None else ox_val,
        "bader_charge": metal_bader if metal_bader is not None else (o_bader if o_bader is not None else generic_bader),
        "d_band_center": metal_dband if metal_dband is not None else generic_dband,
        ...
        "descriptor_atom_scope": clean_scalar(raw.get("descriptor_atom_scope")) or ("atom_resolved" if (o_cn is not None or o_bader is not None or metal_cn is not None or metal_bader is not None or metal_dband is not None) else "generic_or_unclear"),
        "descriptor_assignment_confidence": normalize_assignment_confidence(raw.get("descriptor_assignment_confidence")),
        "descriptor_assignment_note": clean_scalar(raw.get("descriptor_assignment_note")),
    ...
    numeric_keys = [
        "coordination_number", "bader_charge", "d_band_center", "work_function", "PZC",
        "o_site_coordination_number", "o_site_bader_charge", "o_h_bond_length",
        "metal_coordination_number", "metal_bader_charge", "metal_d_band_center", "metal_o_distance", "band_gap"
    ]
    d, desc_support = validate_numeric_fields(d, numeric_keys, ev_txt)
    # Oxidation states may be expressed in roman numerals; allow text support.
    if d.get("metal_oxidation_state") is not None and not (number_supported(d.get("metal_oxidation_state"), ev_txt) or text_supported(d.get("metal_oxidation_state_raw"), ev_txt)):
        d["metal_oxidation_state"] = None
    ...
    desc_support["metal_oxidation_state"] = "supported" if d.get("metal_oxidation_state") is not None else "missing"
...
def build_canonical(paper_id: str, relpath: str, target: Dict[str, Any], desc: Dict[str, Any], idx: int) -> Dict[str, Any]:
    ...
    rec: Dict[str, Any] = {
        ...
        "h_binding_anchor_atom": normalize_anchor_atom((desc.get("h_binding_anchor_atom") if desc else None) or target.get("h_binding_anchor_atom") or target.get("active_atom") or target.get("site_label")),
        "h_binding_site_label": sanitize_site_like((desc.get("h_binding_site_label") if desc else None) or target.get("h_binding_site_label") or target.get("site_label")),
        "neighbor_atoms": [],
        "o_site_coordination_number": desc.get("o_site_coordination_number") if desc else None,
        "o_site_bader_charge": desc.get("o_site_bader_charge") if desc else None,
        "o_site_charge_transfer": desc.get("o_site_charge_transfer") if desc else None,
        "o_h_bond_length": desc.get("o_h_bond_length") if desc else None,
        "o_site_descriptor_claim": desc.get("o_site_descriptor_claim") if desc else None,
        "neighbor_metal_element": desc.get("neighbor_metal_element") if desc else None,
        "neighbor_metal_site_label": desc.get("neighbor_metal_site_label") if desc else None,
        "metal_coordination_number": desc.get("metal_coordination_number") if desc else None,
        "metal_oxidation_state": desc.get("metal_oxidation_state") if desc else None,
        "metal_oxidation_state_raw": desc.get("metal_oxidation_state_raw") if desc else None,
        "metal_oxidation_state_element": desc.get("metal_oxidation_state_element") if desc else None,
        "metal_bader_charge": desc.get("metal_bader_charge") if desc else None,
        "metal_d_band_center": desc.get("metal_d_band_center") if desc else None,
        "metal_o_distance": desc.get("metal_o_distance") if desc else None,
        "band_gap": desc.get("band_gap") if desc else None,
        "descriptor_atom_scope": desc.get("descriptor_atom_scope") if desc else None,
        "descriptor_assignment_confidence": desc.get("descriptor_assignment_confidence") if desc else None,
        "descriptor_assignment_note": desc.get("descriptor_assignment_note") if desc else None,
...
def process_paper(row: Dict[str, Any], args: argparse.Namespace, llm: Optional[MultiEndpointClient]) -> Tuple[str, str, int, Optional[str]]:
    ...
    if args.heuristic_fallback:
        ...
        for t in target_rows:
            tid = t.get("target_id")
            if tid not in desc_map and any(fallback_desc.get(k) is not None for k in ["coordination_number", "bader_charge", "d_band_center", "work_function", "PZC", "oxidation_state_value", "o_site_coordination_number", "o_site_bader_charge", "o_h_bond_length", "metal_coordination_number", "metal_oxidation_state", "metal_bader_charge", "metal_d_band_center", "band_gap", "vacancy_nearby", "hydroxylated_surface", "bridge_structure_nearby"]):
