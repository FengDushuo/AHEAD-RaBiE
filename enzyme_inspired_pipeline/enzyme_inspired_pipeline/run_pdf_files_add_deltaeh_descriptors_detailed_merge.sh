...
ADD_OUTDIR="${ADD_OUTDIR:-outputs_scnet_v4flash_add_deltaeh_detailed}"
ADD_LOGDIR="${ADD_LOGDIR:-logs_scnet_v4flash_add_deltaeh_detailed}"
...
ADD_CANONICAL="$ADD_OUTDIR/hads_canonical_records_add_deltaeh_detailed.jsonl"
MERGED_CANONICAL="${MERGED_CANONICAL:-$ADD_OUTDIR/hads_canonical_records_merged_with_add_detailed.jsonl}"
MERGED_DB="${MERGED_DB:-outputs_scnet_v4flash_merged_add_deltaeh_detailed_db}"
MERGED_STEP5="${MERGED_STEP5:-outputs_scnet_v4flash_merged_add_deltaeh_detailed_step5_stats_ml}"
MERGED_FIGS="${MERGED_FIGS:-outputs_scnet_v4flash_merged_add_deltaeh_detailed_publication_figures}"
...
PY
log "[CONFIG] detailed extraction: workers=$WORKERS_ADD target_tokens=$TARGET_MAX_TOKENS desc_tokens=$DESC_MAX_TOKENS energy_abs_max=$ENERGY_ABS_MAX_EV"
...
run_step "Detailed two-stage ΔE_H* + descriptor extraction" "$ADD_LOGDIR/stepB_detailed_extract_deltaeh_descriptors.log" \
  "$PYTHON_BIN" 3g_detailed_extract_deltaeh_descriptor_joint.py \
    ...
    --out "$ADD_CANONICAL" \
    --progress "$ADD_OUTDIR/progress_3g_detailed_extract_deltaeh_descriptor_joint.json" \
    "${EXTRACT_FORCE_ARGS[@]}"
...
out.to_csv("$ADD_OUTDIR/merged_DeltaEH_descriptor_joint_coverage_detailed.csv", index=False)
...
log "[RESULT] coverage CSV: $ADD_OUTDIR/merged_DeltaEH_descriptor_joint_coverage_detailed.csv"
