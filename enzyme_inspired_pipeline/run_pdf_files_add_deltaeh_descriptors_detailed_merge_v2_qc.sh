...
ADD_OUTDIR="${ADD_OUTDIR:-outputs_scnet_v4flash_add_deltaeh_detailed_qc}"
...
ADD_CANONICAL="$ADD_OUTDIR/hads_canonical_records_add_deltaeh_detailed_qc.jsonl"
MERGED_CANONICAL="${MERGED_CANONICAL:-$ADD_OUTDIR/hads_canonical_records_merged_with_add_detailed_qc.jsonl}"
MERGED_DB="${MERGED_DB:-outputs_scnet_v4flash_merged_add_deltaeh_detailed_qc_db}"
MERGED_STEP5="${MERGED_STEP5:-outputs_scnet_v4flash_merged_add_deltaeh_detailed_qc_step5_stats_ml}"
MERGED_FIGS="${MERGED_FIGS:-outputs_scnet_v4flash_merged_add_deltaeh_detailed_qc_publication_figures}"
...
run_step "Detailed two-stage ΔE_H* + descriptor extraction" "$ADD_LOGDIR/stepB_detailed_extract_deltaeh_descriptors.log" \
  "$PYTHON_BIN" 3g_detailed_extract_deltaeh_descriptor_joint_v2_qc.py \
    ...
    --out "$ADD_CANONICAL" \
    --progress "$ADD_OUTDIR/progress_3g_detailed_extract_deltaeh_descriptor_joint_v2_qc.json" \
    "${EXTRACT_FORCE_ARGS[@]}"
