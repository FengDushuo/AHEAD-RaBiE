...
BAD_MATERIAL_RE = re.compile(r"^(?:X\d+|S\d+|H\d+|I\d+|Hs\d+|Fig\.?\d*|Table\.?\d*|NH2|OH|H2O|DFT|DOS|PDOS|HER|HBE)$", re.I)
...
def is_bad_material_name(s: Optional[str]) -> bool:
    if not s:
        return True
    ss = normalize_text(s)
    if BAD_MATERIAL_RE.match(ss):
        return True
    if len(ss) <= 3 and re.match(r"^[A-Za-z]+\d+$", ss):
        return True
    if ss.lower() in {"reported catalyst", "surface", "catalyst", "sample", "model", "system"}:
        return True
    return False
...
def postprocess_target(raw: Dict[str, Any], evs: List[Dict[str, Any]], ev_text_all: str, paper_id: str, idx: int, max_abs_ev: float) -> Optional[Dict[str, Any]]:
    ...
    rec["H_adsorption_energy_unit"] = clean_unit(rec.get("H_adsorption_energy_unit"))
...
def heuristic_targets(paper_id: str, evs: List[Dict[str, Any]], relpath: str, max_abs_ev: float) -> List[Dict[str, Any]]:
    ...
    for i, m in enumerate(RE_H_E.finditer(txt), 1):
        ...
        pp = postprocess_target(raw, evs, txt, paper_id, i, max_abs_ev)
        if pp:
            out.append(pp)
    return out
...
def normalize_descriptor(raw: Dict[str, Any], evs: List[Dict[str, Any]], ev_text_all: str) -> Dict[str, Any]:
    ...
    d = {
        ...
        "active_atom": clean_scalar(raw.get("active_atom")) or ox_el2 or ox_el,
        ...
        "descriptor_link_method": clean_scalar(raw.get("descriptor_link_method")) or "two_stage_same_paper_link",
...
def build_canonical(paper_id: str, relpath: str, target: Dict[str, Any], desc: Dict[str, Any], idx: int) -> Dict[str, Any]:
    ...
    rec: Dict[str, Any] = {
        ...
        "surface_facet": clean_scalar(target.get("surface_facet")),
        "termination": None,
        "phase": None,
        "support": clean_scalar(target.get("support")),
        "dopant": clean_scalar(target.get("dopant")),
        "interface_type": None,
        "defect_type": clean_scalar(target.get("defect_type")) or clean_scalar(desc.get("defect_type")) if desc else clean_scalar(target.get("defect_type")),
        ...
        "ads_site_type": clean_scalar(target.get("ads_site_type")),
        "site_label": clean_scalar(desc.get("site_label")) if desc and desc.get("site_label") else clean_scalar(target.get("site_label")),
        "active_atom": clean_scalar(desc.get("active_atom")) if desc and desc.get("active_atom") else clean_scalar(target.get("active_atom")),
        "neighbor_atoms": [],
        ...
        "adsorption_configuration": clean_scalar(target.get("adsorption_configuration")),
        ...
        "descriptor_link_method": desc.get("descriptor_link_method") if desc else "target_only_no_descriptor",
...
def process_paper(row: Dict[str, Any], args: argparse.Namespace, llm: Optional[MultiEndpointClient]) -> Tuple[str, str, int, Optional[str]]:
    ...
    if isinstance(target_obj, dict) and isinstance(target_obj.get("targets"), list):
        for i, raw in enumerate(target_obj.get("targets", []), 1):
            pp = postprocess_target(raw, target_evs, ev_target_all, paper_id, i, args.energy_abs_max_ev)
            if pp:
