...
def output_all_files(filename,research_label,key_label,research_list,country_list,journal_list,timeline_list,labels,column_name_only_list,output_path):
    ...
    sentence_model = SentenceTransformer('paraphrase-MiniLM-L6-v2',cache_folder=current_dir+'/paraphrase-MiniLM-L6-v2')
    kw_model = KeyBERT(model=sentence_model)  # 可选择其他模型
    # kw_model = KeyBERT()
    for key, value in timeline_abstract_json.items():
        ...
        for keyword, score in keywords:
            word_dict = {"keyword": keyword, "count": round(score * 100)}  
