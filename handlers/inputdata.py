...
def import_excel_to_sqlite(excel_file, sqlite_path='static/db/articles.db', table_name='articles'):
...
def output_all_files(filename,research_label,key_label,research_list,country_list,journal_list,timeline_list,labels,column_name_only_list,output_path):
    ...
    df = pd.read_excel(filename, engine='openpyxl' if ext.endswith('x') else None, usecols=add_labels, keep_default_na=False) 
    # 示例调用，保存到你想要的位置
    import_excel_to_sqlite(filename, sqlite_path="static/data/"+base+".db")
...
class Inputdata_ipsHandler(BaseHandler):
    ...
    def post(self):
        ...
        # data_input.to_csv("upload/"+base+ ".csv", index=False,  encoding='utf-8')
        #用户选择的标签列表
        ...
        column_name_list=[custom_want['text'][0].decode('utf-8'),custom_want['text'][1].decode('utf-8'),custom_want['text'][2].decode('utf-8'),custom_want['text'][3].decode('utf-8'),"Article Title","Authors","Since 2013 Usage Count","Addresses","Source Title","Research Areas","Keyword List","Publication Year"]
