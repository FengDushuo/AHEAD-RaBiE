...
def loadAbstractFile(file_name):
    ...
    data_cy = data.copy()
    comments = data_cy['Abstract']
    targets = data_cy['Highly Cited Status']
    # 转换标签: 'Y' -> 1, 'nan' -> 0
    targets = targets.apply(lambda x: 1 if x == 'Y' else 0 if pd.isna(x) else x)
    all_sentences = np.array([[comments[i],targets[i]] for i in range(len(comments))])
    print('Step1:read {} abstracts in file...'.format(len(all_sentences)))
    #sentence = ' '.join(comments)
    all_word_train = []
    all_words_dictionary_train = []
    for sentence in all_sentences:
        ...
        all_words_dictionary_train.extend(filtered_words)
    all_words_dictionary_train=list(set(all_words_dictionary_train))
    target_train=all_sentences[:,1].astype(int)  # 强制转换为 int 类型
    return np.array(all_word_train,dtype=object), target_train, all_words_dictionary_train                                                               #返回numpy数组形式的所有abstract内容及对应标签
...
def getWord2Vec(all_words,output,filename): 
    ...
    model = Word2Vec(all_words, sg=0, size=300, window=5, min_count=1, iter=7, negative=10)
...
def run_models(input_file,output):
    ...
    for item in methods_list:
        if item["Train Score"]+item["Test Score"]>max_fitted_algorithm_score:
            max_fitted_algorithm_score = item["Train Score"]+item["Test Score"]
...
# word2vec_model,max_fitted_algorithm_model=run_models("D:/a_work/1-phD/project/7-bibliometric/WOSDataVisualization/upload/savedrecs1000.xls","model_test/8_models_output_2/")
