...
def yake_extractor(text):
    ...
    Returns: list of keywords (list)
    """
    keywords = yake.KeywordExtractor(lan="en", n=3, windowsSize=3).extract_keywords(text)
    results = []
...
def keybert_extractor(text):
    ...
    Returns: list of keywords (list)
    """
    keywords = bert.extract_keywords(text, keyphrase_ngram_range=(3, 5), stop_words="english")
    results = []
    for scored_keywords in keywords:
        for keyword in scored_keywords:
            if isinstance(keyword, str):
...
def benchmark(corpus, shuffle=True):
    ...
    df.to_csv("keyword-performance-results.csv", index=False)
