# https://cloud.tencent.com/developer/article/1909262
...
def rake_extractor(text):
    """
    Uses Rake to extract the top 100 keywords from a text
    Arguments: text (str)
...
def yake_extractor(text):
    """
    Uses YAKE to extract the top 100 keywords from a text
    Arguments: text (str)
    Returns: list of keywords (list)
    """
...
def position_rank_extractor(text):
    """
    Uses PositionRank to extract the top 100 keywords from a text
    Arguments: text (str)
    Returns: list of keywords (list)
    """
    ...
    extractor.candidate_selection(maximum_word_number=100)
...
def single_rank_extractor(text):
    """
    Uses SingleRank to extract the top 100 keywords from a text
    Arguments: text (str)
    Returns: list of keywords (list)
    """
    pos = {'NOUN', 'PROPN', 'ADJ', 'ADV'}
...
def multipartite_rank_extractor(text):
    """
    Uses MultipartiteRank to extract the top 100 keywords from a text
    Arguments: text (str)
    Returns: list of keywords (list)
    """
...
def topic_rank_extractor(text):
    """
    Uses TopicRank to extract the top 100 keywords from a text
    Arguments: text (str)
    Returns: list of keywords (list)
    """
...
def keybert_extractor(text):
    """
    Uses KeyBERT to extract the top 100 keywords from a text
    Arguments: text (str)
    Returns: list of keywords (list)
    """
...
xlsopenfilepath = "D:/a_work/1-phD/project/3-bibliometric/data-sports-injury-medline-26307-20241115/savedrecs2000.xls"
data_input = pd.read_excel(xlsopenfilepath,sheet_name=0)
column_addresses_list=data_input["Abstract"].to_list()
text_list=[x for x in column_addresses_list if pd.isnull(x) == False]
results = benchmark(text_list, shuffle=False)